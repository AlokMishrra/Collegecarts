...
export default function ShiftSelector({ open, onSelectShift, onCancel }) {
  ...
  useEffect(() => {
    if (open) {
      setIsLoading(true);
      // Check if DeliveryShift entity exists
      if (base44.entities.DeliveryShift) {
        ...
        console.error("DeliveryShift entity not found in base44");
        setShifts([]);
        setIsLoading(false);
      }
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-gray-500">
            Choose a shift to start receiving orders
          </DialogDescription>
        </DialogHeader>
