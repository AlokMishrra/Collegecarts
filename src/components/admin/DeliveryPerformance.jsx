...
export default function DeliveryPerformance() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const performanceData = personsData.map(person => {
        const personOrders = ordersData.filter(o => o.delivery_person_id === person.id);
        return {
          delivery_person_id: person.id,
          total_deliveries: personOrders.length,
          avg_rating: person.rating || 0,
          on_time_percentage: 95 // Default value
        };
      });
