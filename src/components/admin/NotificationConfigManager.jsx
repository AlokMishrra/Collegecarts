...
export default function NotificationConfigManager() {
  ...
  const loadConfigs = async () => {
    try {
      // Settings table doesn't have 'key' column - use localStorage
  ...
  const saveConfig = async (config) => {
    try {
      // Save to localStorage since notification_config table doesn't exist
      const updatedConfigs = configs.filter(c => c.event_type !== config.event_type);
      updatedConfigs.push(config);
      localStorage.setItem('notification_configs', JSON.stringify(updatedConfigs));
