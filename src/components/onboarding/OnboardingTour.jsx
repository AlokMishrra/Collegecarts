  ...
  const checkOnboarding = async () => {
    ...
    try {
      ...
      if (records.length === 0) {
        ...
        const role = user.role === 'admin' ? 'admin' : user.role === 'delivery_person' ? 'delivery' : 'support';
        ...
        try {
          const newProgress = await base44.entities.OnboardingProgress.create({
            ...
            role,
            completed_steps: [],
            current_step: ONBOARDING_STEPS[role]?.[0]?.id || 'dashboard',
            is_completed: false
          });
          ...
          // Don't show onboarding if we can't create the record
        }
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <p className="text-sm text-gray-600 mt-2">
            Let's take a quick tour of the admin dashboard features
          </p>
        </DialogHeader>
