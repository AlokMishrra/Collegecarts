  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (categoryId) {
        ...
        // Show all products - cron job handles is_available flag
        // Products will show as "Out of Stock" when is_available is false
  ...
  const addToCart = async (product) => {
    ...
    try {
      ...
      if (existingItem) {
        ...
        const createdItem = await base44.entities.CartItem.create({
          product_id: product.id,
          user_id: user.id,
          quantity: 1
        });
        ...
        if (createdItem) {
          setCartItems(prev => prev.map(item =>
            item.id === tempItem.id ? { ...item, id: createdItem.id } : item
          ));
        }
      }
