...
export default function UserManagement() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const allUsers = await User.list('-created_at');
  ...
  const toggleUserRole = async (userId, roleId) => {
    try {
      ...
      await User.update(userId, { assigned_role_ids: newRoles });
      loadData();
                ...
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1 text-sm text-gray-600">
                      ...
                      {new Date(user.created_at || user.created_date).toLocaleDateString('en-IN', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric'
                      })}
