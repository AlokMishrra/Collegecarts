-- Drop ALL existing overloads of these functions regardless of signature,
-- then recreate with UUID parameter type only.

DROP FUNCTION IF EXISTS public.reserve_stock(TEXT, INT, TEXT);
DROP FUNCTION IF EXISTS public.reserve_stock(TEXT, INT);
DROP FUNCTION IF EXISTS public.reserve_stock(UUID, INT, TEXT);
DROP FUNCTION IF EXISTS public.reserve_stock(UUID, INT);

DROP FUNCTION IF EXISTS public.deliver_stock(TEXT, INT, TEXT);
DROP FUNCTION IF EXISTS public.deliver_stock(TEXT, INT);
DROP FUNCTION IF EXISTS public.deliver_stock(UUID, INT, TEXT);
DROP FUNCTION IF EXISTS public.deliver_stock(UUID, INT);

DROP FUNCTION IF EXISTS public.release_stock(TEXT, INT);
DROP FUNCTION IF EXISTS public.release_stock(UUID, INT);

DROP FUNCTION IF EXISTS public.decrement_stock(TEXT, INT);
DROP FUNCTION IF EXISTS public.decrement_stock(UUID, INT);

-- Now create with UUID — matches products.id column type exactly
...
CREATE FUNCTION public.deliver_stock(
  p_product_id UUID,
  p_quantity   INT,
  p_hostel     TEXT DEFAULT NULL
) RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER AS $body$
...
CREATE FUNCTION public.release_stock(
  p_product_id UUID,
  p_quantity   INT
) RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER AS $body$
BEGIN
