/**
 * reset-password — Admin-reset a user's password after OTP verification
 */
...
serve(async (req) => {
  ...
  try {
    ...
    if (!foundUser) {
      return Response.json({ error: 'No account found with this email' }, { status: 404, headers: corsHeaders });
    }
    ...
    if (!updateRes.ok) {
      return Response.json({ error: updateData.msg || 'Failed to update password' }, { status: 500, headers: corsHeaders });
    }
